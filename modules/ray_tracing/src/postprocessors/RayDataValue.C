...
registerMooseObject("MooseApp", RayDataValue);
...
{
  const auto id_set = parameters.isParamSetByUser("ray_id");
  const auto name_set = parameters.isParamSetByUser("ray_name");
  ...
  if (name_set && !_study.useRayRegistration())
    paramError("study",
               _study.type(),
...
{
  // Get the Ray ID
  if (parameters().isParamSetByUser("ray_name"))
  {
    const auto & ray_name = getParam<std::string>("ray_name");
...
RayDataValue::getValue()
{
  ...
  if (_aux)
    return _study.getBankedRayAuxData(_ray_id, _ray_data_index);
  else
