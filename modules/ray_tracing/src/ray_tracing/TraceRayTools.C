...
bool
onBoundingBoxBoundary(const BoundingBox & bbox, const Point & point, const unsigned int dim)
{
  for (unsigned int d = 0; d < dim; ++d)
    if (MooseUtils::absoluteFuzzyEqual(point(d), bbox.min()(d)) ||
        MooseUtils::absoluteFuzzyEqual(point(d), bbox.max()(d)))
      return true;
