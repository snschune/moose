                              ...
                              Real & intersection_distance)
{
  ...
  for (MooseIndex(neighbor_info._sides.size()) i = 0; i < neighbor_info._sides.size(); ++i)
    if (neighbor_info._side_normals[i] * (*_current_ray)->direction() < TRACE_TOLERANCE)
    {
  ...
  do
  {
    ...
    if (_dim > 1 && _intersected_extrema.atExtrema() &&
        ...
        (!_is_rectangular_domain ||
         onBoundingBoxBoundary(_study.boundingBox(), _intersection_point, _dim)))
    {
      ...
      if (boundary_elem)
      {
        ...
        // we don't need it to continue the trace. This is the primary reason
        // why we don't allow _incoming_side and ray->currentIncomingSide()
        // to be available outside of TraceRay (in RayKernels/RayBCs)
        _last_elem = _current_elem;
...
{
  traceAssert(_current_elem->contains_point(ray->currentPoint()), "Elem doesn't contain point");
