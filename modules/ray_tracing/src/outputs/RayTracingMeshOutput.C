...
RayTracingMeshOutput::output(const ExecFlagType & type)
{
  ...
  _cached_traces = _study.getCachedTraces();
  if (!_cached_traces)
    mooseError(this->type(),
               " '",
               name(),
               "': Cached trace segments not available in ",
               _study.type(),
               " '",
               _study.name(),
               "'.");
  ...
  auto num_segments = _cached_traces->size();
  ...
  // Remove our claim to the cached traces
  _cached_traces.reset();
}
...
RayTracingMeshOutput::buildIDMap()
{
  ...
  for (const auto & trace_data : *_cached_traces)
  {
...
RayTracingMeshOutput::buildSegmentMesh()
{
  ...
  mooseAssert(_cached_traces, "Cached traces unavailable");
  ...
  for (const auto & entry : *_cached_traces)
  {
