...
A RayKernel operates on a line segment of a [Ray.md], which is defined by the entry and exit point of the [Ray.md] within an element as it is traced through the mesh. The base object is the `RayKernelBase`.
...
The parameter `bool at_end` in `onSegment()` denotes whether or not `_current_segment_end` is at the Ray's max distance to trace. This is either because:
...
TODO: add link to more detailed doc on modifying/registering

If your RayKernel requires data or auxiliary data on a [Ray.md] that is unique to said object, you can register said requirement in the constructor using `_study.registerRayData()` and `_study.registerAuxData()`.
...
By registering the data, as long as you are appropriately sizing your generated [Ray.md]s (via `RayTracingStudy::rayDataSize()` and `RayTracingStudy::rayAuxDataSize()`, done automatically when generating a [Ray.md] within a RayKernel or RayBC), the data can be obtained by `currentRay().data()` and `currentRay().auxData()`.
...
First, acquire a new [Ray.md] using the `acquireRay()` method, which takes as arguments the starting point and direction of travel for the new [Ray.md]. For example:
