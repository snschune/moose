...
public:
  ReceiveBuffer(const libMesh::Parallel::Communicator & comm,
                Context & context,
                MooseUtils::Buffer<std::shared_ptr<Object>> & working_object_buffer,
...
private:
  ...
  Context & _context;
  /// The working buffer
  MooseUtils::Buffer<std::shared_ptr<Object>> & _working_object_buffer;
...
ReceiveBuffer<Object, Context>::ReceiveBuffer(
    ...
    Context & context,
    MooseUtils::Buffer<std::shared_ptr<Object>> & working_buffer,
    const ParallelStudyMethod method,
    const unsigned int clicks_per_receive,
    const Parallel::MessageTag object_buffer_tag)
  : ParallelObject(comm),
    _context(context),
    _working_object_buffer(working_buffer),
    _method(method),
...
ReceiveBuffer<Object, Context>::cleanupRequests()
{
  _requests.remove_if([&](std::pair<std::shared_ptr<Parallel::Request>,
    ...
    if (req->test()) // See if the receive has completed
    {
      ...
      _working_object_buffer.append(objects->begin(), objects->end());

      // Release our ownership of these objects
      objects->clear();
