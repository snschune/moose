...
public:
   ...
   * be able to obatin the element normals often enough during a trace, one needs to implement a
   ...
   *
   * This is ONLY guaranteed to valid during execution of RayKernels!
   */
...
private:
   ...
   */
  std::string failTraceMessage(const std::string & reason);
   ...
   */
  void failTrace(const std::string & reason, const bool warning = false);
   ...
   * @param skip_side The side of elem to skip for intersection checks
   ...
   * @param intersected_extrema To be modified with with the intersected extrema
   * (vertex/edge), if any
   ...
   * @param elem Any element that currently contains the Ray (for building neighbors, if needed)
   * @param at_vertex The vertex the Ray is currently known to be at, if any
   * @param within_vertices The vertices the Ray is currently known to be within, if any
   * @param skip_elem An element to skip in the check, if any
   * @param best_elem The best element to move through, if any
   * @param best_elem_incoming_side The side of the best neighbor found to move through, if any
   ...
   * Get the neighbors at an edge.
   * @param elem An elem that contains the edge
   * @param vertices A pair of local vertex IDs of the vetices that contain the edge fully
   * @param point The point on the edge where neighbors are desired
   */
  const std::vector<NeighborInfo> &
  getEdgeNeighbors(const Elem * elem,
...
#define traceAssert(asserted, msg)                                                                 \
  ...
  {                                                                                                \
    if (!(asserted))                                                                               \
      mooseAssert(asserted, failTraceMessage(msg));                                                \
  } while (0)
#endif
