...
RayData
TestRayDataStudy::dataValue(const unsigned int i, const std::shared_ptr<Ray> & ray) const
{
  Point actual_start;
...
RayData
TestRayDataStudy::auxDataValue(const unsigned int i, const std::shared_ptr<Ray> & ray) const
{
  Point actual_start;
  actual_start(0) = ray->auxData(_actual_start_indices[0]);
  actual_start(1) = ray->auxData(_actual_start_indices[1]);
...
TestRayDataStudy::modifyRays()
{
  ...
  {
    ray->data().resize(rayDataSize());
    ray->auxData().resize(rayAuxDataSize());

    for (unsigned int i = 0; i < 3; ++i)
      ray->auxData(_actual_start_indices[i]) = ray->startPoint()(i);
...
TestRayDataStudy::onCompleteRay(const std::shared_ptr<Ray> & ray)
{
  ...
  for (unsigned int i = 0; i < _data_size; ++i)
    if (!MooseUtils::absoluteFuzzyEqual(ray->data(_data_indices[i]), dataValue(i, ray)))
      mooseError(_name, ": Incorrect expected data");

  for (unsigned int i = 0; i < _aux_data_size; ++i)
    if (!MooseUtils::absoluteFuzzyEqual(ray->auxData(_aux_data_indices[i]), auxDataValue(i, ray)))
